    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Track AI visibility for each brand</p>
        </div>
        <button className="btn-primary w-fit shrink-0" onClick={() => setShowForm(s => !s)}>
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {brand.industry && (
                    <span className="text-xs text-slate-400 mt-0.5 block">{brand.industry}</span>
                  )}
                </div>
                <span className="text-gray-500 group-hover:text-indigo-400 transition-colors text-lg leading-none">→</span>
              </div>
              {brand.description && (
                <p className="text-slate-400 text-sm mt-3 line-clamp-2">{brand.description}</p>
              )}
