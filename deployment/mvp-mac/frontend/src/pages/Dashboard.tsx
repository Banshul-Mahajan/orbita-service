...
export default function Dashboard() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white mb-1">{selectedProject.name}</h1>
          ...
          {steps.map((s) => (
            <Link
              ...
              className={clsx(
                ...
                s.done
                  ? 'bg-emerald-950/40 border border-emerald-800/40 hover:border-emerald-700/60'
                  : 'bg-gray-800/60 border border-gray-700/50 hover:border-gray-600 hover:bg-gray-800'
              )}
            >
              <span className={clsx(
                'text-xs font-mono w-6 shrink-0 font-semibold',
                s.done ? 'text-emerald-500' : 'text-slate-500'
              )}>{s.step}</span>
              <div className={clsx(
                'p-1.5 rounded-md shrink-0',
                s.done ? 'bg-emerald-900/50 ring-1 ring-emerald-700/50' : 'bg-gray-700/60'
              )}>
                {s.icon}
              </div>
              <div className="flex-1 min-w-0">
                <p className={clsx(
                  ...
                  s.done ? 'text-emerald-300' : 'text-gray-100'
                )}>
                  {s.title}
                  {s.done && (
                    <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-emerald-900 text-emerald-300 border border-emerald-700/60">done</span>
                  )}
                </p>
                <p className={clsx(
              ...
              <ArrowRight size={14} className={clsx(
                ...
                s.done ? 'text-emerald-600 group-hover:text-emerald-400' : 'text-gray-600 group-hover:text-gray-300'
              )} />
            </Link>
          ))}
        </div>
