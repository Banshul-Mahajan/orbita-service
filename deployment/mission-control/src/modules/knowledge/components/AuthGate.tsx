  ...
  return (
    <div style={pageStyle}><div style={cardStyle}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        <div style={{ width: 44, height: 44, borderRadius: 10, background: 'linear-gradient(135deg,#8b5cf6,#06b6d4)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 16 }}>KC</div>
        <div><div style={{ fontSize: 18, fontWeight: 700, color: '#e2e8f0' }}>Knowledge Core</div><div style={{ fontSize: 12, color: '#64748b' }}>{mode === 'login' ? 'Sign in' : 'Create account'}</div></div>
      </div>
...
function BrandStep({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => { authClient.get('/brands').then(r => { setBrands(r.data || []); setLoading(false) }).catch(() => setLoading(false)) }, [])

  const selectBrand = (b: Brand) => { setCookie('orbit_brand_id', b.id); onDone() }
  const createBrand = async () => {
    if (!brandName.trim()) return; setCreating(true); setError('')
    try { const { data } = await authClient.post('/brands', { name: brandName, industry }); setCookie('orbit_brand_id', data.id); onDone() }
    catch (err: any) { setError(err.response?.data?.detail || 'Failed'); setCreating(false) }
  }
  ...
  return (
    <div style={pageStyle}><div style={cardStyle}>
      ...
      <p style={{ fontSize: 13, color: '#64748b', marginBottom: 16 }}>Choose the brand to manage knowledge for.</p>
      {error && <div style={errorStyle}>{error}</div>}
...
// ─── Styles ──────────────────────────────────────────────────────────────────
const pageStyle: React.CSSProperties = { minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0f1a' }
const cardStyle: React.CSSProperties = { background: '#111827', border: '1px solid #1f2937', borderRadius: 16, padding: 36, width: '100%', maxWidth: 440 }
const inputStyle: React.CSSProperties = { padding: '10px 14px', background: '#0a0f1a', border: '1px solid #1f2937', borderRadius: 8, color: '#e2e8f0', fontSize: 14, outline: 'none', width: '100%', boxSizing: 'border-box' }
const labelStyle: React.CSSProperties = { fontSize: 13, color: '#94a3b8', marginBottom: 2 }
const btnStyle: React.CSSProperties = { padding: 12, border: 'none', borderRadius: 8, background: 'linear-gradient(135deg,#8b5cf6,#06b6d4)', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer' }
const errorStyle: React.CSSProperties = { background: '#7f1d1d30', border: '1px solid #991b1b', borderRadius: 8, padding: '8px 12px', marginBottom: 16, color: '#fca5a5', fontSize: 13 }
